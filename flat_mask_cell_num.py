...
def main():
    ...
    root_dir = "gs://octopi-malaria-data-processing/malaria-tanzina-2021/"#'gs://octopi-codex-data-processing/' #"/home/prakashlab/Documents/kmarx/pipeline/tstflat/"# 'gs://octopi-codex-data-processing/TEST_1HDcVekx4mrtl0JztCXLn9xN6GOak4AU/'#
...
def run_seg(root_dir, exp_id, channel, zstack, cpmodel, channels, key, use_gpu, segment_all, gcs_project):
    ...
    csvpath = root_dir + exp_id + "count.csv"
    csvtemp = './count.csv'
    if root_remote:
        csvpath = csvtemp
    with open(csvpath, 'w') as c:
        c.write("\"view path\",count\n")
    ...
    for idx, impath in enumerate(segpaths):
        ...
        if root_remote:
            ...
            if segment_all:
                savepath = dest + impath.rsplit('.', 1)[0]
            else:
                savepath = dest + "first" + '/' + impath.rsplit('.', 1)[0]
            os.makedirs(savepath, exist_ok=True)
            savepath = savepath + impath.split('/')[-1]
        # save the data
        image_path = savepath + "_mask.bmp"
        bool_mask = (masks >= 1) * 255
        count = np.max(masks)
        with open(csvpath, 'a') as c:
            splt =  impath.split('/')
            imsplt = splt[-1].split("_")
            i = imsplt[0]
            j = imsplt[1]
            c.write(f"{i}, {j}, {count}\n")

        cv2.imwrite(image_path, bool_mask)

        # move the .npy to remote if necessary
        if root_remote:
            # generate the local and remote path names
            rpath ="gs://" + imgpaths[idx].rsplit(".", 1)[0] + "_mask.bmp"
            fs.put(image_path, rpath)
            print(image_path)
            print(rpath)
    ...
    if model_remote:
        os.remove(modelpath)
    if root_remote:
        rpath = root_dir + exp_id + 'count.csv'
        print(csvpath)
        fs.put(csvpath, rpath)
        print(rpath)
        os.remove(csvpath)
